...
namespace md
{
	...
	GSPipeline::~GSPipeline()
	{
		vkDestroyShaderModule(gsDevice.device(), vertShaderModule, nullptr);
		vkDestroyShaderModule(gsDevice.device(), pixelShaderModule, nullptr);
		vkDestroyPipeline(gsDevice.device(), graphicsPipeline, nullptr);
	}
	...
	void GSPipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& pixelFilepath, const PipelineConfigInfo& configInfo)
	{
		...
		createShaderModule(pixelCode, &pixelShaderModule);
		...
		if (vkCreateGraphicsPipelines(gsDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
	...
	void GSPipeline::createShaderModule(const std::vector<char>& code, VkShaderModule* shaderModule)
	{
		...
		if (vkCreateShaderModule(gsDevice.device(), &createInfo, nullptr, shaderModule) != VK_SUCCESS)
	...
	void GSPipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo)
	{
		...
		configInfo.rasterizationInfo.depthClampEnable = VK_FALSE; // clamps depth values to 0 or 1
